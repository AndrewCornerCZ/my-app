...
export default function ActivitiesMapPage() {
  ...
  // Inicializuj user icon - POUZE NA KLIENTU
  useEffect(() => {
  ...
  useEffect(() => {
    if (!mapContainer.current || !userLocation || mapInstance.current || typeof window === 'undefined') return
  ...
  useEffect(() => {
    if (!mapInstance.current || !userLocation || !userIcon || typeof window === 'undefined') return
  ...
  return (
    <div className="w-full h-screen flex flex-col bg-gray-950">
      ...
      <div className="relative z-20 flex flex-col sm:flex-row items-start sm:items-center justify-between px-4 lg:px-5 py-3 bg-gray-950/95 backdrop-blur-md border-b border-white/10 flex-shrink-0 gap-2 lg:gap-4">
        <div className="flex items-center gap-2 lg:gap-3">
          ...
          <div>
            ...
            <h1 className="text-white font-extrabold text-xs lg:text-sm leading-none">Activities Near Me</h1>
          </div>
        </div>
      ...
      {/* ── Filters Panel ── */}
      {showFilters && (
        <div className="relative z-20 bg-gray-950/95 backdrop-blur-md border-b border-white/10 px-4 lg:px-5 py-3 lg:py-4 space-y-3 lg:space-y-4 flex-shrink-0">
          <div>
              ...
              <div className="flex flex-wrap gap-2">
                {availableSports.map(sport => (
                  <button key={sport.id} onClick={() => handleSportToggle(sport.id)}
                    className={`px-2.5 lg:px-3 py-1 lg:py-1.5 rounded-lg lg:rounded-xl border text-xs font-medium transition-all duration-200 ${
                ...
                {selectedSports.length > 0 && (
                  <button onClick={() => { setSelectedSports([]); applyFilters(allActivities, []) }}
                    className="px-2.5 lg:px-3 py-1 lg:py-1.5 rounded-lg lg:rounded-xl border border-red-500/30 text-red-400 text-xs font-medium hover:bg-red-500/10 transition-all duration-200">
                    Clear
                  </button>
                )}
              </div>
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        {/* Sidebar */}
        {showSidebar && (
          <aside className="hidden lg:flex flex-col w-64 lg:w-72 border-r border-white/10 bg-gray-950/95 backdrop-blur-md overflow-hidden flex-shrink-0">
            <div className="px-3 lg:px-4 py-2.5 lg:py-3 border-b border-white/10 flex items-center justify-between flex-shrink-0">
              <div>
                <p className="text-xs font-semibold uppercase tracking-widest text-teal-500">Nearby</p>
                <p className="text-white font-bold text-xs lg:text-sm">{activities.length} activities</p>
              </div>
              <span className="text-xs text-gray-600 bg-white/5 border border-white/10 px-2 py-1 rounded-lg">{radius} km</span>
            </div>

            <div className="flex-1 overflow-y-auto px-2 lg:px-3 py-2 lg:py-3 space-y-1.5 lg:space-y-2">
              {activities.length === 0 ? (
                <div className="flex flex-col items-center justify-center h-32 text-center">
                  <p className="text-gray-600 text-xs lg:text-sm">No activities found</p>
                ...
                activities.map(activity => (
                  <button
                    ...
                    className={`w-full text-left px-2.5 lg:px-3 py-2.5 lg:py-3 rounded-xl lg:rounded-2xl border transition-all duration-200 ${
                  ...
                  >
                    <div className="flex items-start gap-2">
                      <span className="text-lg leading-none mt-0.5">{activity.sport.emoji}</span>
        ...
        <div className="flex-1 relative">
          ...
          {/* Count badge */}
          <div className="absolute bottom-3 lg:bottom-4 left-3 lg:left-4 z-10 bg-gray-950/90 backdrop-blur-md border border-white/10 text-white px-3 lg:px-4 py-2 lg:py-2.5 rounded-lg lg:rounded-2xl shadow-xl">
          ...
          {selectedActivity && (
            <div className="absolute bottom-3 lg:bottom-4 right-3 lg:right-4 z-10 w-72 bg-gray-950/95 backdrop-blur-md border border-teal-500/30 text-white rounded-xl lg:rounded-2xl shadow-2xl p-3 lg:p-4">
              <div className="flex items-start justify-between gap-2 mb-2">
                <div className="flex items-center gap-1.5 lg:gap-2 flex-1 min-w-0">
                  <span className="text-xl lg:text-2xl flex-shrink-0">{selectedActivity.sport.emoji}</span>
                  <div className="min-w-0">
                    <p className="font-bold text-xs lg:text-sm">{selectedActivity.sport.name}</p>
                ...
                <button onClick={() => setSelectedActivity(null)} className="text-gray-600 hover:text-white transition-colors mt-0.5 flex-shrink-0">
          ...
          {!loading && activities.length === 0 && (
            <div className="absolute inset-0 flex items-end justify-center pb-16 lg:pb-20 pointer-events-none z-10">
              <div className="bg-gray-950/90 backdrop-blur-md border border-white/10 text-white px-4 lg:px-6 py-3 lg:py-4 rounded-xl lg:rounded-2xl shadow-xl text-center">
                ...
                <p className="text-xs text-gray-500 mt-0.5 lg:mt-1">Try increasing the radius or clearing filters</p>
              </div>
            </div>
          )}
        </div>
